...
cdef class CostSensitiveTreeBuilder:
    """Interface for different tree building strategies."""

    cpdef build(self, Tree tree, object X, np.ndarray y, object cost_mat,
                np.ndarray sample_weight=None):
        """Build a decision tree from the training set (X, y, cost_mat)."""
        pass
...
cdef class DepthFirstCostSensitiveTreeBuilder(CostSensitiveTreeBuilder):
    """Build a decision tree in depth-first fashion."""

    def __cinit__(self, CostSensitiveSplitter splitter, SIZE_t min_samples_split,
                  SIZE_t min_samples_leaf, double min_weight_leaf,
                  SIZE_t max_depth, double min_impurity_decrease):
        self.splitter = splitter
        self.min_samples_split = min_samples_split
    ...
    cpdef build(self, Tree tree, object X, np.ndarray y, object cost_mat,
        ...
        # Parameters
        cdef CostSensitiveSplitter splitter = self.splitter
        cdef SIZE_t max_depth = self.max_depth
        cdef SIZE_t min_samples_leaf = self.min_samples_leaf
        cdef double min_weight_leaf = self.min_weight_leaf
        cdef SIZE_t min_samples_split = self.min_samples_split
        ...
        # Recursive partition (without actual recursion)
        splitter.init(X, y, cost_mat, sample_weight_ptr)
