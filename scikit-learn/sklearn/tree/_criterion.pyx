...
cdef class CostSensitiveCriterion:
    """Interface for impurity criteria.
    ...
    def __dealloc__(self):
        ...
        free(self.sum_left)
        free(self.sum_right)
        free(self.sum_cost_total)
        free(self.sum_cost_left)
        free(self.sum_cost_right)
    ...
    cdef int init(self, const DOUBLE_t[:, ::1] y, object cost_mat, DOUBLE_t* sample_weight,
                  double weighted_n_samples, SIZE_t* samples, SIZE_t start,
                  SIZE_t end) nogil except -1:
        """Placeholder for a method which will initialize the criterion.
...
cdef class CostSensitiveClassificationCriterion(CostSensitiveCriterion):
    ...
    def __cinit__(self, SIZE_t n_outputs,
        ...
        self.sum_right = NULL
        self.n_classes = NULL
        self.sum_cost_total = NULL
        self.sum_cost_left = NULL
        self.sum_cost_right = NULL
        ...
        cdef SIZE_t n_combinations = n_classes[0] * n_classes[0]
        self.sum_cost_total = <double*> calloc(n_combinations, sizeof(double))
        self.sum_cost_left = <double*> calloc(n_combinations, sizeof(double))
        self.sum_cost_right = <double*> calloc(n_combinations, sizeof(double))

        if (self.sum_total == NULL or
                self.sum_left == NULL or
                self.sum_right == NULL or 
                self.sum_cost_total == NULL or
                self.sum_cost_left == NULL or
                self.sum_cost_right == NULL):
            raise MemoryError()
    ...
    cdef int init(self, const DOUBLE_t[:, ::1] y, object cost_mat,
                  DOUBLE_t* sample_weight, double weighted_n_samples,
                  SIZE_t* samples, SIZE_t start, SIZE_t end) nogil except -1:
        """Initialize the criterion.
        ...
        cdef SIZE_t* n_classes = self.n_classes
        cdef double* sum_total = self.sum_total
        cdef double* sum_cost_total = self.sum_cost_total
        ...
        for k in range(n_classes[0]):
            memset(sum_cost_total + offset, 0, n_classes[0] * sizeof(double))
            offset += n_classes[0]

        #printf("func: init\n")
        #printf("func: init:: start: %d\n", self.start)
        #printf("func: init:: end: %d\n", self.end)

        for p in range(start, end):
            i = samples[p]
            #printf("func: init:: i: %d\n", i)
            ...
            for c in range(n_classes[0]):
                y_i = <SIZE_t> self.y[i, 0]
                sum_cost_total[c * n_classes[0] + y_i] += w * <DOUBLE_t> self.cost_mat[i, y_i * n_classes[0] + c]
                #printf("func: init:: c: %d \t %d\n", c, y_i)
                #printf("func: init:: cost_mat: %f\n", self.cost_mat[i, y_i * n_classes[0] + c])
                #printf("func: init:: %f\n", sum_cost_total[0 * n_classes[0] + 0])
                #printf("func: init:: %f\n", sum_cost_total[0 * n_classes[0] + 1])
                #printf("func: init:: %f\n", sum_cost_total[1 * n_classes[0] + 0])
                #printf("func: init:: %f\n", sum_cost_total[1 * n_classes[0] + 1])
    ...
    cdef int reset(self) nogil except -1:
        ...
        cdef double* sum_right = self.sum_right
        cdef double* sum_cost_total = self.sum_cost_total
        cdef double* sum_cost_left = self.sum_cost_left
        cdef double* sum_cost_right = self.sum_cost_right

        cdef SIZE_t* n_classes = self.n_classes
        cdef SIZE_t k

        for k in range(self.n_outputs):
            #printf("func: reset:: Sum Total: %f\n", sum_total[0])
            #printf("func: reset:: Sum Total: %f\n", (sum_total + 1)[0])

            #printf("func: reset:: Sum Left: %f\n", sum_left[0])
            #printf("func: reset:: Sum Left: %f\n", (sum_left + 1)[0])
            memset(sum_left, 0, n_classes[k] * sizeof(double))
            #printf("func: reset:: Sum Left: %f\n", sum_left[0])
            #printf("func: reset:: Sum Left: %f\n", (sum_left + 1)[0])

            #printf("func: reset:: Sum Right: %f\n", sum_right[0])
            #printf("func: reset:: Sum Right: %f\n", (sum_right + 1)[0])
            memcpy(sum_right, sum_total, n_classes[k] * sizeof(double))
            #printf("func: reset:: Sum Right: %f\n", sum_right[0])
            #printf("func: reset:: Sum Right: %f\n", (sum_right + 1)[0])
        ...
        for k in range(n_classes[0]):
            #printf("func: reset:: Sum Cost Total: %f\n", sum_cost_total[0])
            #printf("func: reset:: Sum Cost Total: %f\n", (sum_cost_total + 1)[0])

            #printf("func: reset:: Sum Cost Left: %f\n", sum_cost_left[0])
            #printf("func: reset:: Sum Cost Left: %f\n", (sum_cost_left + 1)[0])
            memset(sum_cost_left, 0, n_classes[0] * sizeof(double))
            #printf("func: reset:: Sum Cost Left: %f\n", sum_cost_left[0])
            #printf("func: reset:: Sum Cost Left: %f\n", (sum_cost_left + 1)[0])

            #printf("func: reset:: Sum Cost Right: %f\n", sum_cost_right[0])
            #printf("func: reset:: Sum Cost Right: %f\n", (sum_cost_right + 1)[0])
            memcpy(sum_cost_right, sum_cost_total, n_classes[0] * sizeof(double))
            #printf("func: reset:: Sum Cost Right: %f\n", sum_cost_right[0])
            #printf("func: reset:: Sum Cost Right: %f\n", (sum_cost_right + 1)[0])

            sum_cost_total += n_classes[0]
            sum_cost_left += n_classes[0]
            sum_cost_right += n_classes[0]
    ...
    cdef int reverse_reset(self) nogil except -1:
        ...
        for k in range(n_classes[0]):
            memset(sum_cost_right, 0, n_classes[0] * sizeof(double))
            memcpy(sum_cost_left, sum_cost_total, n_classes[0] * sizeof(double))
    ...
    cdef int update(self, SIZE_t new_pos) nogil except -1:
        ...
        # sum_left from the direction that require the least amount
        # of computations, i.e. from pos to new_pos or from end to new_po.
        #printf("func: update:: start: %d\n", self.start)
        #printf("func: update:: pos: %d\n", pos)
        #printf("func: update:: new pos: %d\n", new_pos)
        #printf("func: update:: end: %d\n", end)
        if (new_pos - pos) <= (end - new_pos):
            #printf("forward\n")
            for p in range(pos, new_pos):
                i = samples[p]
                #printf("func: update:: i: %d\n", i)

                if sample_weight != NULL:
                    w = sample_weight[i]
                    #printf("func: update:: w: %f\n", w)

                for k in range(self.n_outputs):
                    label_index = k * self.sum_stride + <SIZE_t> self.y[i, k]
                    sum_left[label_index] += w
                    #printf("func: update:: Sum Left: %f\n", sum_left[0])
                    #printf("func: update:: Sum Left: %f\n", sum_left[1])

                # c is the predicted class
                for c in range(n_classes[0]):
                    y_i = <SIZE_t> self.y[i, 0]
                    sum_cost_left[c * n_classes[0] + y_i] += w * <DOUBLE_t> self.cost_mat[i, y_i * n_classes[0] + c]
                    #printf("func: update:: predicted: %d\n", c)
                    #printf("func: update:: gt: %d\n", y_i)
                    #printf("func: update:: Sum Cost Left: %f\n", sum_cost_left[0])
                    #printf("func: update:: Sum Cost Left: %f\n", (sum_cost_left + 1)[0])
                    #printf("func: update:: Sum Cost Left: %f\n", (sum_cost_left + 2)[0])
                    #printf("func: update:: Sum Cost Left: %f\n", (sum_cost_left + 3)[0])
            ...
            for p in range(end - 1, new_pos - 1, -1):
                ...
                if sample_weight != NULL:
                    w = sample_weight[i]
                    #printf("func: update:: rev. w: %f\n", w)

                for k in range(self.n_outputs):
                    label_index = k * self.sum_stride + <SIZE_t> self.y[i, k]
                    sum_left[label_index] -= w
                    #printf("func: update:: rev. Sum Left: %f\n", sum_left[0])
                    #printf("func: update:: rev. Sum Left: %f\n", sum_left[1])

                for c in range(n_classes[0]):
                    y_i = <SIZE_t> self.y[i, 0]
                    sum_cost_left[c * n_classes[0] + y_i] -= w * <DOUBLE_t> self.cost_mat[i, y_i * n_classes[0] + c]
                    #printf("func: update:: rev. predicted: %d\n", c)
                    #printf("func: update:: rev. gt: %d\n", y_i)
                    #printf("func: update:: rev. Sum Cost Left: %f\n", sum_cost_left[0])
                    #printf("func: update:: rev. Sum Cost Left: %f\n", (sum_cost_left + 1)[0])
                    #printf("func: update:: rev. Sum Cost Left: %f\n", (sum_cost_left + 2)[0])
                    #printf("func: update:: rev. Sum Cost Left: %f\n", (sum_cost_left + 3)[0])
        ...
        for k in range(n_classes[0]):
            for c in range(n_classes[0]):
                sum_cost_right[c] = sum_cost_total[c] - sum_cost_left[c]

            sum_cost_right += n_classes[0]
            sum_cost_left += n_classes[0]
            sum_cost_total += n_classes[0]

        sum_cost_left = self.sum_cost_left
        # printf("func: update:: Sum Cost Left: %f\n", sum_cost_left[0])
        # printf("func: update:: Sum Cost Left: %f\n", (sum_cost_left + 1)[0])
        # printf("func: update:: Sum Cost Left: %f\n", (sum_cost_left + 2)[0])
        # printf("func: update:: Sum Cost Left: %f\n", (sum_cost_left + 3)[0])
        sum_cost_right = self.sum_cost_right
        # printf("func: update:: Sum Cost Right: %f\n", sum_cost_right[0])
        # printf("func: update:: Sum Cost Right: %f\n", (sum_cost_right + 1)[0])
        # printf("func: update:: Sum Cost Right: %f\n", (sum_cost_right + 2)[0])
        # printf("func: update:: Sum Cost Right: %f\n", (sum_cost_right + 3)[0])
        sum_cost_total = self.sum_cost_total
        # printf("func: update:: Sum Cost Total: %f\n", sum_cost_total[0])
        # printf("func: update:: Sum Cost Total: %f\n", (sum_cost_total + 1)[0])
        # printf("func: update:: Sum Cost Total: %f\n", (sum_cost_total + 2)[0])
        # printf("func: update:: Sum Cost Total: %f\n", (sum_cost_total + 3)[0])
    ...
    cdef void node_value(self, double* dest) nogil:
        ...
        # printf("func: node_value:: Classwise Sum Cost Total: %f\n", sum_total[0])
        # printf("func: node_value:: Classwise Sum Cost Total: %f\n", (sum_total + 1)[0])

        for c in range(n_classes[0]):
            sum_total[c] = 0
            for gt in range(n_classes[0]):
                sum_total[c] -= sum_cost_total[c * n_classes[0] + gt]
...
cdef class MisclassificationCost(CostSensitiveClassificationCriterion):
    ...
    cdef double node_impurity(self) nogil:
        ...
        # printf("func: node_impurity:: Sum Cost Total: %f\n", sum_cost_total[0])
        # printf("func: node_impurity:: Sum Cost Total: %f\n", (sum_cost_total + 1)[0])
        # printf("func: node_impurity:: Sum Cost Total: %f\n", (sum_cost_total + 2)[0])
        # printf("func: node_impurity:: Sum Cost Total: %f\n", (sum_cost_total + 3)[0])
        ...
        for c in range(n_classes[0]):
            ...
            for j in range(n_classes[0]):
                cost_c += sum_cost_total[c * n_classes[0] + j]
                # printf("func: node_impurity:: C: %d\n", c)
                # printf("func: node_impurity:: GT: %d\n", j)
                # printf("func: node_impurity:: Cost of Predict GT as C: %f\n", sum_cost_total[c * n_classes[0] + j])

            # saving the class with least cost
            if cost_c < best_cost:
                best_cost_class = c
                best_cost = cost_c

            # printf("func: node_impurity:: Best Class to Predict: %d\n", c)
            # printf("func: node_impurity:: Cost of Predicting: %f\n", cost_c)
    ...
    cdef void children_impurity(self, double* impurity_left,
        ...
        cdef double* sum_cost_left = self.sum_cost_left
        cdef double* sum_cost_right = self.sum_cost_right
        cdef double cost_c_left
        cdef double cost_c_right
        cdef double best_cost_left
        cdef double best_cost_right
        cdef SIZE_t best_cost_left_class
        cdef SIZE_t best_cost_right_class
        cdef SIZE_t c
        cdef SIZE_t j

        best_cost_left = INFINITY
        best_cost_right = INFINITY
        best_cost_left_class = -1
        best_cost_right_class= -1

        # printf("func: children_impurity:: Sum Cost Left: %f\n", sum_cost_left[0])
        # printf("func: children_impurity:: Sum Cost Left: %f\n", (sum_cost_left + 1)[0])
        # printf("func: children_impurity:: Sum Cost Left: %f\n", (sum_cost_left + 2)[0])
        # printf("func: children_impurity:: Sum Cost Left: %f\n", (sum_cost_left + 3)[0])
        # printf("func: children_impurity:: Sum Cost Right: %f\n", sum_cost_right[0])
        # printf("func: children_impurity:: Sum Cost Right: %f\n", (sum_cost_right + 1)[0])
        # printf("func: children_impurity:: Sum Cost Right: %f\n", (sum_cost_right + 2)[0])
        # printf("func: children_impurity:: Sum Cost Right: %f\n", (sum_cost_right + 3)[0])
        ...
        for c in range(n_classes[0]):
            ...
            # printf("func: children_impurity:: C: %d\n", c)
            # printf("func: children_impurity:: Sum Left: %f\n", sum_left[c])
            # printf("func: children_impurity:: Sum Right: %f\n", sum_right[c])
            ...
            # j - gt
            for j in range(n_classes[0]):
                cost_c_left += sum_cost_left[c * n_classes[0] + j]
                cost_c_right += sum_cost_right[c * n_classes[0] + j]
                # printf("func: children_impurity:: J: %d\n", j)
                # printf("func: children_impurity:: sum_cost_left[c * n_classes[0] + j]: %f\n", sum_cost_left[c * n_classes[0] + j])
                # printf("func: children_impurity:: sum_cost_right[c * n_classes[0] + j]: %f\n", sum_cost_right[c * n_classes[0] + j])

            # saving the class with least cost
            if cost_c_left < best_cost_left:
                best_cost_left_class = c
                best_cost_left = cost_c_left

            # saving the class with least cost
            if cost_c_right < best_cost_right:
                best_cost_right_class = c
                best_cost_right = cost_c_right

        # printf("func: children_impurity:: Best Class to Predict (left): %d\n", best_cost_left_class)
        # printf("func: children_impurity:: Cost of Predicting (left): %f\n", best_cost_left)
        # printf("func: children_impurity:: Best Class to Predict (right): %d\n", best_cost_right_class)
        # printf("func: children_impurity:: Cost of Predicting (right): %f\n", best_cost_right)

        impurity_left[0] = best_cost_left
        impurity_right[0] = best_cost_right
