...
from ._criterion cimport Criterion, CostSensitiveCriterion
...
cdef class CostSensitiveSplitter:
    """Abstract splitter class.
    ...
    def __cinit__(self, CostSensitiveCriterion criterion, SIZE_t max_features,
                  SIZE_t min_samples_leaf, double min_weight_leaf,
                  object random_state):
        """
        Parameters
    ...
    cdef int node_reset(self, SIZE_t start, SIZE_t end,
        ...
        self.init_criterion(start, end)

        weighted_n_node_samples[0] = self.criterion.weighted_n_node_samples
        return 0

    cpdef int init_criterion(self, SIZE_t start, SIZE_t end):
        self.criterion.init(
            self.y,
            self.cost_mat,
            self.sample_weight,
...
cdef class BaseDenseCostSensitiveSplitter(CostSensitiveSplitter):
    cdef const DTYPE_t[:, :] X
    ...
    cdef int init(self,
        ...
        # Call parent init
        CostSensitiveSplitter.init(self, X, y, cost_mat, sample_weight)
...
cdef class BestCostSensitiveSplitter(BaseDenseCostSensitiveSplitter):
    """Splitter for finding the best split."""
    def __reduce__(self):
        return (BestSplitter, (self.criterion,
                               self.max_features,
