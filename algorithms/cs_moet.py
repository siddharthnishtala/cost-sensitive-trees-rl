...
from .cs_viper import train_dagger, split_train_test, build_cost_matrix
...
def train_cs_moet(env, black_box_policy, config):
...
class MOEPolicy:
    ...
    def _fit(self, obss, acts, qs, obss_test, acts_test, qs_test):
    ...
    def train(self, obss, qs, train_frac):

        acts = np.argmax(qs, axis=1)

        (
            obss_train, acts_train, qs_train, 
            obss_test, acts_test, qs_test
        ) = split_train_test(
            obss, acts, qs, train_frac
        )
        self._fit(obss_train, acts_train, qs_train, obss_test, acts_test, qs_test)
...
class MOETClassifier(MOETBase):
    ...
    def _fit_epoch(
        ...
        for j in range(self.experts_no):
            ...
            if max_depth == 0:
                self.dtc_list[j] = ExampleDependentCostSenstiveDecisionTreeClassifier(
                    max_depth=1, min_samples_split=len(x_normalized) + 1
                )
            else:
                self.dtc_list[j] = ExampleDependentCostSenstiveDecisionTreeClassifier(max_depth=max_depth)

            x_normalized = x_normalized.astype(np.float32)
            costs = costs.astype(np.float32)

            cost_mat = build_cost_matrix(costs)

            self.dtc_list[j].fit(x_normalized, y, cost_mat, sample_weight=weights[:, j].T)
...
class MOETClassifierNew(MOETClassifier):
    ...
    def train_experts(self, x_normalized, y, costs, pdf, max_depth):

        for j in range(self.experts_no):
            if max_depth == 0:
                self.dtc_list[j] = ExampleDependentCostSenstiveDecisionTreeClassifier(max_depth=1,
    ...
    def _fit_epoch(self,
        ...
        self.train_experts(x_normalized, y, costs, pdf, max_depth)
